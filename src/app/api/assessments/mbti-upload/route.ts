...
async function extractMBTITypeWithAI(buffer: Buffer, fileName: string): Promise<string | null> {
  try {
    ...
    try {
      ...
      // Try Chat Completions API directly with the file first (more reliable)
      console.log('Trying Chat Completions API with file...');
      try {
        const chatResponse = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: 'system',
              content: `You are an expert at extracting MBTI personality types from PDF documents. Your task is to find the MBTI personality type in the provided PDF document.
...
If you cannot find a valid MBTI type, return "NOT_FOUND".`
        ...
        const responseText = chatResponse.choices[0]?.message?.content?.trim().toUpperCase() || '';
        console.log('Chat Completions API response:', responseText);

        // Extract MBTI type from response
        const validMBTITypes = [
          'ENFJ', 'ENFP', 'ENTJ', 'ENTP',
          'ESFJ', 'ESFP', 'ESTJ', 'ESTP',
          'INFJ', 'INFP', 'INTJ', 'INTP',
        ...
        // Check if response contains a valid MBTI type
        for (const type of validMBTITypes) {
          if (responseText.includes(type)) {
            extractedType = type;
            console.log('Found MBTI type in Chat Completions response:', extractedType);
            break;
          }
        }
        ...
        console.log('Chat Completions API did not find MBTI type, trying Assistants API as fallback...');
      } catch (chatError: any) {
        console.log('Chat Completions API failed:', chatError.message);
        console.log('Falling back to Assistants API...');
      }

      // Fallback to Assistants API if Chat Completions fails
      console.log('Using Assistants API as fallback...');
